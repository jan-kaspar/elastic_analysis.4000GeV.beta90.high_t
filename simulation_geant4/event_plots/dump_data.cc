...
int main()
{
	...
	// get input
	string topDir = "../../";
	//string tag = "_2E5_seed1";
	string tag = "_flat_1E5_seed1";
	//string tag = "_th_y_low_3E4_seed2";
	//string tag = "_th_y_high_2E4_seed2";

	TChain *ch = new TChain("ntuple");
	ch->Add((topDir + "ntuple_sim_hits" + tag + ".root").c_str());

	TChain *ch_full = new TChain("TotemNtuple");
	ch_full->Add((topDir + "ntuple_full" + tag + ".root").c_str());

	TChain *ch_ideal = new TChain("TotemNtuple");
	ch_ideal->Add((topDir + "ntuple_ideal" + tag + ".root").c_str());
